...
def person_search_api(request):
    ...
    cache_key = f"person_search:{keyword}:{page}:{page_size}"
    ...
    try:
        ...
        if is_pinyin:
            cursor.execute(
                """SELECT r.RSID, r.XM AS 姓名, r.RYBH AS 档案编号, r.CSNY, r.WORKTIME,
            ...
            cursor.execute(f"""SELECT r.RSID, r.XM AS 姓名, r.RYBH AS 档案编号, r.CSNY, r.WORKTIME,
                                      ISNULL(d.BMMC, '') AS 单位名称
                               FROM RS_INFO r
                               LEFT JOIN USERS_DEPARTMENT ud ON r.RSID = ud.RSID
                               LEFT JOIN DEPART d ON ud.DEPARTMENTID = d.BM
            ...
            results.append(
                {
                    ...
                    "csny": r.get("CSNY", "") or "",
                    "worktime": r.get("WORKTIME", "") or "",
                    "matchType": "拼音匹配" if is_pinyin else "",
                }
            )

        cache.set(cache_key, {"data": results, "total": total}, 300)
