...
def person_panel_embed(request):
    path = os.path.join(settings.BASE_DIR, 'templates', 'components', 'person_panel_embed.html')
    with open(path, 'r', encoding='utf-8') as f:
        return HttpResponse(f.read())


def archive_tree_embed(request):
    path = os.path.join(settings.BASE_DIR, 'templates', 'components', 'archive_tree_embed.html')
    with open(path, 'r', encoding='utf-8') as f:
        return HttpResponse(f.read())

def unit_tree_embed(request):
    path = os.path.join(settings.BASE_DIR, 'templates', 'components', 'unit_tree_embed.html')
    with open(path, 'r', encoding='utf-8') as f:
        return HttpResponse(f.read())
