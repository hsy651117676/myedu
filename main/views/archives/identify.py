...
def identify_save_api(request):
    ...
    rsid = data.get("rsid")
    if not rsid: return JsonResponse({"code": 400, "msg": "缺少RSID"})
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE RSID = ?
        """, (
            data.get('RDBa2', ''), data.get('RDBa3', ''),
            data.get('RDBb2', ''), data.get('RDBb3', ''),
            data.get('RDBc3', ''),
            full_rdbd1, rdbd2, int(rsid)
        ))
        ...
        if str(old_csny or '') != str(new_csny):
            cursor.execute("INSERT INTO Z_Log_rsinfo (userid, rsid, OperationType, oldvalue, newvalue, classNumber, mtime) VALUES (?, ?, 1, ?, ?, '出生年月', GETDATE())", (yhbh, int(rsid), str(old_csny or ''), str(new_csny)))
        ...
        if str(old_wt or '') != str(new_wt):
            cursor.execute("INSERT INTO Z_Log_rsinfo (userid, rsid, OperationType, oldvalue, newvalue, classNumber, mtime) VALUES (?, ?, 1, ?, ?, '参工时间', GETDATE())", (yhbh, int(rsid), str(old_wt or ''), str(new_wt)))
        ...
        cursor.execute("""
            ...
            WHERE RSID = ?
        """, (
            data.get('RDBa1', ''), data.get('RDBb1', ''), data.get('RDBc1', ''),
            data.get('RDBe1', ''), data.get('RDBe2', ''), data.get('RDBe3', ''),
            data.get('RDBe4', ''), data.get('RDBe5', ''), data.get('RDBe6', ''),
            data.get('RDBe7', ''), int(rsid)
        ))
...
def identify_generate_api(request):
    ...
    try:
        ...
        cursor.execute("SELECT lx, name, createTime, totalTime FROM YW_Alteration WHERE rsid=?", (int(rsid),))
        ...
        def fmt6(s):
            if not s: return ''
            s = str(s)
            if len(s) >= 6: return s[:4] + '年' + str(int(s[4:6])) + '月'
            return s

        def fmt8(s):
            if not s: return ''
            s = str(s)
            if len(s) >= 8: return s[:4] + '年' + str(int(s[4:6])) + '月' + str(int(s[6:8])) + '日'
            return s
        ...
        def fmt_long(s):
            ...
            result = []
            for t in times:
                if len(t) == 6: result.append(t[:4] + '年' + str(int(t[4:6])) + '月')
                elif len(t) == 4: result.append(t + '年')
        ...
        csny_alt = ''; cjgz_alt = ''
        for alt in alt_rows:
            lx = alt[0] or ''; mat = alt[1] or ''; t1 = fmt8(alt[2]); t2 = fmt6(alt[3])
            desc = f'{t1}形成的《{mat}》中有涂改的情况，涂改后'
            if lx == '出生年月': csny_alt += desc + f'出生年月为{t2}。'
            elif lx == '参加工作时间': cjgz_alt += desc + f'参加工作时间为{t2}。'
...
def alteration_save_api(request):
    ...
    rsid = data.get("rsid")
    rows = data.get("rows", [])
    if not rsid or not rows: return JsonResponse({"code": 400})
    ...
    try:
        ...
        cursor = conn.cursor()
        for row in rows:
            xh = row.get("序号", 0); lx = row.get("涂改类型", "")
            name = row.get("被涂改材料名称", ""); ct = row.get("被涂改材料形成时间", "")
            tt = row.get("涂改后时间", "")
            cursor.execute("SELECT COUNT(*) FROM YW_Alteration WHERE rsid=? AND xh=?", (int(rsid), int(xh)))
            if cursor.fetchone()[0] > 0:
                cursor.execute("UPDATE YW_Alteration SET lx=?, name=?, createTime=?, totalTime=? WHERE rsid=? AND xh=?", (lx, name, ct, tt, int(rsid), int(xh)))
            else:
                cursor.execute("INSERT INTO YW_Alteration (rsid, xh, lx, name, createTime, totalTime) VALUES (?,?,?,?,?,?)", (int(rsid), int(xh), lx, name, ct, tt))
...
def alteration_delete_api(request):
    rsid = request.GET.get('rsid', ''); xh = request.GET.get('xh', '')
    if not rsid or not xh: return JsonResponse({"code": 400})
    conn = None
    try:
        conn = _get_conn(); cursor = conn.cursor()
        cursor.execute("DELETE FROM YW_Alteration WHERE rsid=? AND xh=?", (int(rsid), int(xh)))
        cursor.execute("UPDATE YW_Alteration SET xh=xh-1 WHERE rsid=? AND xh>?", (int(rsid), int(xh)))
        conn.commit(); cursor.close()
...
def identify_export_api(request):
    ...
    template_path = os.path.join(settings.BASE_DIR, 'static', 'excel_templates', '认定表.xlsx')
    ...
    try:
        ...
        # 认定数据
        cursor.execute("""
            SELECT a.rdyj, a.zzbmyj
            FROM YW_ZXSHDJ a WHERE a.RSID = ?
        """, (int(rsid),))
