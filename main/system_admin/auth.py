    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            try:
                ...
                return JsonResponse(
                    {"code": 400, "msg": "请求数据格式错误"}, status=400
                )
            if required_fields:
                missing = [f for f in required_fields if not data.get(f, "").strip()]
                if missing:
                    return JsonResponse(
                        {"code": 400, "msg": f"缺少必需参数: {', '.join(missing)}"},
                        status=400,
                    )
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if cache.get(lock_key):
            messages.error(
                request, f"账户已锁定，请{LOGIN_LOCKOUT_TIME // 60}分钟后再试"
            )
            return render(request, "auth/login.html")
        ...
        if user is None:
            ...
            if attempts >= MAX_LOGIN_ATTEMPTS:
                ...
                messages.error(
                    request, f"错误次数过多，请{LOGIN_LOCKOUT_TIME // 60}分钟后再试"
                )
            else:
                remaining = MAX_LOGIN_ATTEMPTS - attempts
                messages.error(
                    request,
                    f"用户名或密码错误，还剩 {remaining}/{MAX_LOGIN_ATTEMPTS} 次机会",
                )
...
def change_pwd(request):
    ...
    if not request.user.check_password(old_pwd):
        ...
        return JsonResponse(
            {"code": 400, "msg": f"旧密码错误，剩余 {3 - attempts} 次机会"}
        )
...
def login_api(request):
    ...
    user = authenticate(
        request, username=username, password=password, is_archive_user=True
    )
    if user:
        login(request, user)
